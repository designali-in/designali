...
import { DESIGNALI_GRAFFITI } from "../utils/ascii-arts";
...
const COMPONENT_REGISTERY_URL = "https://designali.in/api/components";
type CompToAddProps = {
  comp_path: string;
  comp_content: string;
};
...
console.log(gradient("pink", "blue")(DESIGNALI_GRAFFITI));

export const add = new Command()

  .name("add")
  .description("add a new component or UI from designali")
  .argument(
    "<string>",
    "id of the component from https://designali.in/components",
  )
  ...
  .action(async (id, opts) => {
    ...
    try {
      ...
      if (framework !== "react") {
        logger.error(
          `We are not currently supporting ${fm} on https://designali.in`,
        );
        logger.info(
          "Be a part of the community by adding an integration to your favorite framework, go for https://github.com/designali-in/designali",
        );
        process.exit(0);
      }
      ...
      const root_dir = path.join(path_, "/designali");
      ...
      try {
        ...
        logger.info(
          "If you think this is an bug, Please make an issue - https://github.com/designali-in/designali",
        );
        process.exit(0);
      }
      comp_spinner.succeed(
        `Found the component. Preview here - https://designali.in/example/${options.id}`,
      );
      comp_spinner.stop();
      const select_files_by_id = comp_db.find((x) => x.id === options.id);
      if (!select_files_by_id) {
        logger.error(
          "No such component exists with in this ID, Please hop on https://designali.in//components to double check the component ID",
        );
        logger.info(
          "If you think this is an bug, Please make an issue - https://github.com/designali-in/designali",
        );
      ...
      if (!supported_fm) {
        logger.info(
          "We have not added the component yet. But you can be the on to add this. hop on https://github.com/designali-in/designali",
        );
        process.exit(0);
      }
