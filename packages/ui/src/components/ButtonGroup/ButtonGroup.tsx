...
function useButtonGroupContext() {
  ...
  if (!context) {
    throw new Error(
      "ButtonGroup.Item must be used within a ButtonGroup or ButtonGroup.Root",
    );
  }
    ...
    return (
      <ButtonGroupContext.Provider value={{ size, disabled, orientation }}>
        <div
          ref={ref}
          className={cn(
            "dark:shadow:none divide-surface-100 border-surface-200 shadow-wg-xs dark:border-surface-100 inline-flex flex-wrap items-stretch rounded-[9px] border",
    ...
    return (
      <Component
        ...
        className={cn(
          ...
          isIcon && size === "sm" && "py-6px px-2 [--wg-border-width:1px]",
          isIcon && size === "md" && "py-8px px-3 [--wg-border-width:1px]",
          className,
        )}
        after={after}
        before={before}
