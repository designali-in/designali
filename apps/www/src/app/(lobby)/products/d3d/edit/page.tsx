...
import { SVGModel } from "@/components/lobby/products/d3d/svg-model"
...
import { GeometryControls } from "@/components/lobby/products/d3d/controls/geometry-controls"
import { MaterialControls } from "@/components/lobby/products/d3d/controls/material-controls"
import { EnvironmentControls } from "@/components/lobby/products/d3d/controls/environment-controls"
import { BackgroundControls } from "@/components/lobby/products/d3d/controls/background-controls"
import { ExportButtons } from "@/components/lobby/products/d3d/export-buttons"
import { EditorMobileWarning } from "@/components/lobby/products/d3d/mobile-warning"

import { MATERIAL_PRESETS, DARK_MODE_COLOR, LIGHT_MODE_COLOR } from "@/lib/constants"
...
const ModelPreview = React.memo<ModelPreviewProps>(
  ({
        ...
        return (
          <EffectComposer multisampling={isMobile ? 0 : 4}>
            <Bloom
              ...
              radius={0.9}
            />
            <BrightnessContrast brightness={0.07} contrast={0.05} blendFunction={BlendFunction.NORMAL} />
          </EffectComposer>
        )
      } else if (!isMobile) {
        return (
    ...
    const environment = useMemo(() => {
      ...
      return <SimpleEnvironment environmentPreset={environmentPreset} customHdriUrl={customHdriUrl} />
    ...
    return (
      ...
      >
        <Suspense fallback={null}>
          ...
          <directionalLight position={[50, 50, 100]} intensity={0.8 * Math.PI} castShadow={false} />
...
export default function EditPage() {
  ...
  const [roughness, setRoughness] = useState<number>(initialPreset?.roughness ?? 0.5)
  const [metalness, setMetalness] = useState<number>(initialPreset?.metalness ?? 0.5)
  const [clearcoat, setClearcoat] = useState<number>(initialPreset?.clearcoat ?? 0)
  const [envMapIntensity, setEnvMapIntensity] = useState<number>(initialPreset?.envMapIntensity ?? 1.0)
  const [transmission, setTransmission] = useState<number>(initialPreset?.transmission ?? 0)
  ...
  useEffect(() => {
    ...
    const savedSelectedIcon = localStorage.getItem("selectedIcon")
    ...
    if (savedSelectedIcon) {
      setSelectedIcon(savedSelectedIcon)
    }
  }, [router])
  ...
  return (
    <motion.main
      className="container-wrapper mt-4 rounded-t-3xl border-t flex flex-col"
      ...
      >
        <div className="container flex items-center justify-between h-16 px-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {svgData && <ExportButtons fileName={fileName} modelGroupRef={modelGroupRef} />}
          </div>
        </div>
      </motion.header>
        ...
        <AnimatePresence mode="wait">
          {isMobile && !continueOnMobile ? (
            <motion.div
              ...
              transition={{ duration: 0.3 }}
            >
              <EditorMobileWarning onContinue={handleContinueOnMobile} onReturn={handleBackToHome} />
            </motion.div>
          ) : (
            <motion.div
              key="editor-content"
            ...
            >
              ...
              >
                <Card className="w-full h-full flex flex-col overflow-hidden border-[1px] shadow-sm">
                  ...
                  <div className="flex-grow relative">{renderModelPreview()}</div>
                </Card>
              </motion.div>

              <motion.div className="space-y-6 order-last lg:order-first" variants={cardAnimation}>
                <Card>
