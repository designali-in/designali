...
export default function Footer({ topic, page, prevPage, nextPage }: Props) {
	...
	const nextPageArea = nextPage ? (
		<Link
			href={`/graphic/${topic}/${nextPage}`}
			className={!prevPage ? 'mx-auto' : 'flex items-center justify-center'}>
			<Button
				variant='link'
	...
	const prevPageArea = prevPage ? (
		<>
			<Link
				href={`/graphic/${topic}/${prevPage}`}
				className={!nextPage ? 'flex items-center justify-center mx-auto' : ''}>
				<Button
					size='icon'
					className='mr-2'
					variant='outline'>
			...
			{pageNums.map((num, i) =>
				page && num === parseInt(page) ? (
					<Button
						...
						className='mx-1'
						size='icon'>
						<span className='block h-5 w-12'>{num}</span>
					</Button>
				) : (
					<Button
						asChild
						...
						className='mx-1'
						variant='outline'>
						<Link href={`/graphic/${topic}/${num}`}>{num}</Link>
					</Button>
				),
			)}
		</>
