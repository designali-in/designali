...
const GalleryHome = async ({ topic = "curated", page }: Props) => {
  let url;
  if (topic === "curated" && page) {
    // browsing beyond home
    url = `https://api.pexels.com/v1/curated?page=${page}&per_page=${perPage}`;
  ...
  return (
    <>
      <ImageContainer3 photosWithBlur={photosWithBlur} /> 
      <Link className="flex justify-center mb-6" href={"/graphic/curated"}>
