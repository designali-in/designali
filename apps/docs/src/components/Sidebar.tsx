...
import type { DocsConfig } from "@/config/sidebarConfig";
import { useId, useState } from "react";
import Link from "next/link";
import { usePathname } from "next/navigation";
import { Badge, Button } from "@designali/ui";
...
export function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        "bg-wg-gray-900/50 fixed inset-y-0 left-0 m-0  md:bg-transparent md:pb-20",
        isSidebarOpen ? "block" : "hidden",
        "z-[60] w-full text-base leading-8 backdrop-blur md:sticky md:top-12 md:z-30 md:block md:h-[calc(100vh-119px)] md:shrink-0 md:text-sm md:leading-6 md:backdrop-blur-none",
      )}
  ...
  return (
    <div className="pl-3">
      <div className="flex items-center justify-between">
        <h3 className="text-surface-900 rounded-md font-medium">
          {item.label}
        </h3>
      ...
      {isOpen ? (
        <SidebarDropdownItems items={item.children} pathname={pathname} />
      ) : null}
    </div>
  );
}
      ...
      {items.map((item, index) =>
        item.href && !item.disabled ? (
          ...
          >
            <Link
              className={cn(
                "hover:bg-surface flex w-full items-center gap-2 rounded-md py-1 pl-9 pr-3 !outline-0",
                pathname === item.href
          ...
          <span
            key={`${id}-${index}`}
            className={cn(
              "text-surface-500 relative flex w-full items-center justify-between rounded-md py-1 pl-9 pr-3 !outline-0 transition-colors last:after:bottom-4",
