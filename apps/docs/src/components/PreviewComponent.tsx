...
import type { TabsElement, TabsProps } from "@designali/ui";
import {
  forwardRef,
  Suspense,
  useEffect,
...
import { Button, Tabs } from "@designali/ui";
  ...
  return (
    ...
    >
      ...
      <Tabs.Content className={cn("rounded-lg", theme)} value="preview">
        <div
          ref={previewRef}
          className="preview not-prose dark:bg-wg-gray-950 border-surface-100 shadow-wg-xs relative flex w-full items-center justify-center rounded-lg border bg-background text-sm text-foreground"
        >
          <div
            className={cn(
              "flex min-h-[300px] w-full items-center overflow-x-auto p-10",
              {
...
function NotFound() {
  return (
    <span className="text-surface-500">
      Unable to display component preview
    </span>
  );
}
