...
import { Alert, Button, Tooltip } from "@designali/ui";
...
function CustomCode(props: HTMLAttributes<HTMLElement>) {
  ...
  return (
    <code
      className={cn(
        "not-prose bg-surface-100/70 text-surface-800 relative rounded px-[0.3rem] py-[0.15rem] font-mono text-[13px] leading-6",
        className,
      )}
      {...otherProps}
    />
...
function CustomPre(props: HTMLAttributes<HTMLPreElement> & { raw?: string }) {
  ...
  return (
    <pre
      className={cn(
        "not-prose bg-wg-gray-900 my-6 max-h-[650px] overflow-x-auto rounded-lg py-4 leading-6",
        className,
      )}
      {...otherProps}
    >
      {children}
      {raw ? (
        <CopyButton className="absolute right-2 top-2" content={raw} />
      ) : null}
    </pre>
  );
}
...
function LinkCard({ className, ...props }: React.ComponentProps<typeof Link>) {
  return (
    <Link
      className={cn(
        "not-prose border-surface-100 text-surface-400 shadow-wg-xs hover:border-surface-200/70 hover:bg-surface-50/50 hover:text-surface-900 flex aspect-[3/2] min-h-[140px] w-full flex-col items-center justify-center gap-4 rounded-xl border bg-white p-6 outline-primary transition-colors duration-75 last-of-type:aspect-auto sm:p-10 [&>p]:text-center",
        className,
      )}
      {...props}
    />
          ...
          rehypePlugins: [
            ...
            [
              rehypeAutolinkHeadings,
              {
                ...
                properties: {
                  className: [
                    "outline-primary size-6 rounded-lg flex items-center justify-center",
                  ],
                },
                headingProperties: {
                  "data-link": "",
