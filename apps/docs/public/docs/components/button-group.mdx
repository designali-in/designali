...
title: Button Group
description:
  A collection of buttons grouped together for related actions enhancing UI
  organization.
links:
...
Alternatively, `ButtonGroup.Root` can be used in place of this component.
Besides all props from `HTMLAttributes<HTMLDivElement>`, `ButtonGroup` also
...
<PropsTable
  content={[
    ...
    [
      {
        value: "disabled",
        description: "Disable all actions on buttons within this group.",
      },
      { value: "boolean" },
      { value: "false" },
    ],
...
Includes all props from the `Button` component, with the exception of `variant`,
`shape`, and `size` props.
...
The following examples shows a combination of the `ButtonGroup` and `Tooltip`
components.
