...
title: Textarea
description:
  UI element for multi-line text input, enabling users to enter data or
  information.
links:
...
In addition to all the props defined as
`TextareaHTMLAttributes<HTMLTextAreaElement>`, `Textarea` component also
...
The textarea in the example below is marked as `required`. It also includes a
`tooltip` and is styled with a `destructive` theme.
