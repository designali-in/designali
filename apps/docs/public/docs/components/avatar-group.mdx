...
For more advanced usage, `AvatarGroup.Root` component can be used to compose
your own group of `AvatarGroup.Item`.
  ...
  content={[
    [
      { value: "items" },
      {
        ...
        description:
          'Omit<AvatarProps, "size" | "notification" | "status" | "asChild">',
      },
      {},
    ],
    [
...
Represents label in the avatar group. In addition to all the props defined as
`HTMLAttributes<HTMLDivElement>`, `AvatarGroup.Label` component also includes
the following props:
...
The following example shows more advanced usage of the `AvatarGroup` component
and how you can customize it with Tailwind CSS classes.

<PreviewComponent name="avatar-group/example-2" />

For automatic inheritance of background color for borders in `AvatarGroup`, use
the `wg-bg-{color}` utility class on parent element instead of `bg-{color}`. For
example:
